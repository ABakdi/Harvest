...
class NotificationPlanner {
  ...
  /// Morning review, evening plan, expense check-in, prime time, and
  /// the streak-risk nudge — the daily rituals behind the master switch.
  ...
  Future<void> _planDebtReminders(DateTime at, AppLocalizations l10n) async {
    final storedRow = await (_db.select(
      _db.kvSettings,
    )..where((s) => s.key.equals('reminders.debtIds'))).getSingleOrNull();
    if (storedRow != null) {
  ...
  Future<void> _planTaskReminders(DateTime at, AppLocalizations l10n) async {
    ...
    final storedRow = await (_db.select(
      _db.kvSettings,
    )..where((s) => s.key.equals('reminders.taskIds'))).getSingleOrNull();
    if (storedRow != null) {
      final ids = (jsonDecode(storedRow.valueJson) as List<dynamic>?) ?? [];
      for (final id in ids) {
        await _notifications.cancel(id as int);
  ...
  Future<DateTime?> _primeTime(DateTime now) async {
    ...
    final minutes =
        firstByDay.values.map((t) => t.hour * 60 + t.minute).toList()..sort();
