...
| 13 | **Nothing leaves the device unless I ask** | The app makes no outbound request of its own. The two that exist are asked for by hand: an export I tap, and an exercise animation I open ([[ADR-008-Exercise-Catalogue]]) — a GET for a static file, carrying no account, no device id and no history. Anything else is a bug, not a feature. |
