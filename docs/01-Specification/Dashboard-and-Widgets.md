...
Per-commitment history (calendar heat-map, streak record, project burn-up chart), plus lifetime XP and rank progress.
...
## Home-screen widgets (Phase 5)

Via `home_widget`:
- **Compact:** streak number + most urgent task.
- **Medium "Vitality":** four mini-gauges — tasks X/Y, sleep hrs, screen X/cap, spent X/limit.
- Tapping opens straight into Quick Log, skipping the home screen.
