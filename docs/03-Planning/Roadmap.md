...
| [[Phase-4-Health-and-Gym]] | Steps, body weight, and a real training log — programs, sessions, personal records — plus the sleep alarm and debt | **In beta as v2.0.0-beta.2** (beta.1 plus [[Checkpoint-6]] and the [[Audit-v2-Beta]] fixes; [[Checkpoint-7]] on top, on `dev`). Passes when I run a full training week from it, and the weight line has a shape |
